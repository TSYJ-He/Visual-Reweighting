...
class AgentConfig:
    ...
    """number of samples per minibatch for updating agent"""
    micro_batch_size_per_device_for_update: int = 4
    """number of samples per forward pass for updating agent"""
    ...
    disable_divergence: bool = field(default=False, init=False)
    use_divergence_loss: bool = field(default=False, init=False)
    divergence_penalty: str = field(default="kl", init=False)
    divergence_coef: float = field(default=0.0, init=False)
