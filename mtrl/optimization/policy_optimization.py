"""
Core optimization functions for policy gradient algorithms.
These functions are used by trainers with different distributed strategies.
"""
...
class DivergenceController(ABC):
    divergence_coef: float
    """Divergence coefficient."""

    @abstractmethod
    def update(self, current_divergence: float, n_steps: int):
        """Update divergence_coef according to current divergence."""
        ...


class AdaptiveDivergenceController(DivergenceController):
    """Adaptive divergence controller described in: https://arxiv.org/pdf/1909.08593.pdf

    Copied from https://github.com/huggingface/trl/blob/v0.11.0/trl/trainer/utils.py#L54"""

    def __init__(self, init_divergence_coef: float, target_divergence: float, horizon: float):
        self.divergence_coef = init_divergence_coef
        self.target = target_divergence
        self.horizon = horizon

    def update(self, current_divergence: float, n_steps: int):
        target = self.target
        proportional_error = np.clip(current_divergence / target - 1, -0.2, 0.2)
        mult = 1 + proportional_error * n_steps / self.horizon
        self.divergence_coef *= mult


class FixedDivergenceController(DivergenceController):
    """Fixed divergence controller.

    Copied from https://github.com/huggingface/trl/blob/v0.11.0/trl/trainer/utils.py#L72"""

    def __init__(self, init_divergence_coef: float):
        self.divergence_coef = init_divergence_coef
...
class AdvantageEstimatorType(str, Enum):
    """
    Using an enumeration class to avoid spelling errors in advantage estimator
    """
...
ADVANTAGE_ESTIMATOR_REGISTRY: dict[str, Any] = {}


def create_divergence_controller(optimization_config: "OptimizationConfig") -> DivergenceController:
    """Adapted from https://github.com/huggingface/trl/blob/v0.11.0/trl/trainer/ppo_trainer.py#L319"""
    if optimization_config.divergence_type == "fixed":
        controller = FixedDivergenceController(init_divergence_coef=optimization_config.divergence_coef)
    elif optimization_config.divergence_type == "adaptive":
        assert optimization_config.divergence_horizon > 0, f"horizon must be larger than 0. Got {optimization_config.divergence_horizon}."
        controller = AdaptiveDivergenceController(
            init_divergence_coef=optimization_config.divergence_coef,
            target_divergence=optimization_config.divergence_target,
            horizon=optimization_config.divergence_horizon,
        )
    else:
        raise ValueError(f"Unknown divergence type: {optimization_config.divergence_type}.")

    return controller


def register_advantage_estimator(name: AdvantageEstimatorType):
    """Decorator to register an advantage estimator function with a given name."""

    def decorator(fn):
        wrapped_fn = torch.no_grad()(fn)
        ADVANTAGE_ESTIMATOR_REGISTRY[getattr(name, "value", name)] = wrapped_fn
        return wrapped_fn

    return decorator


def calculate_advantage_and_return(estimator_name: AdvantageEstimatorType, **kwargs) -> tuple[torch.Tensor, torch.Tensor]:
    """Calculate advantage and return for a given advantage estimator."""
    return ADVANTAGE_ESTIMATOR_REGISTRY[getattr(estimator_name, "value", estimator_name)](**kwargs)


@register_advantage_estimator(AdvantageEstimatorType.GAE)
def calculate_gae_advantage_and_return(
...
@register_advantage_estimator(AdvantageEstimatorType.GRPO)
def calculate_grpo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate advantage for GRPO, operating only on Outcome reward (with only one scalar reward for each response).
...
@register_advantage_estimator(AdvantageEstimatorType.GRPO_PASSK)
def calculate_grpo_passk_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, eps: float = 1e-6, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate advantage for Pass@k using a GRPO-style outcome reward formulation.
...
@register_advantage_estimator(AdvantageEstimatorType.RLOO)
def calculate_rloo_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, index: torch.Tensor, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate advantage for RLOO based on https://arxiv.org/abs/2402.14740
...
@register_advantage_estimator(AdvantageEstimatorType.REINFORCE_PLUS_PLUS)
def calculate_reinforce_plus_plus_outcome_advantage(
    token_level_rewards: torch.Tensor, response_mask: torch.Tensor, gamma: torch.Tensor, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate advantage for REINFORCE++.
...
@register_advantage_estimator(AdvantageEstimatorType.REMAX)
def calculate_remax_outcome_advantage(
    token_level_rewards: torch.Tensor, reward_baselines: torch.Tensor, response_mask: torch.Tensor, **kwargs
) -> tuple[torch.Tensor, torch.Tensor]:
    """
    Calculate advantage for ReMax, operating only on Outcome reward
...
def calculate_rewards(
    ...
    divergence = log_probs - ref_log_probs
    return token_level_scores - divergence * divergence_ratio
...
def calculate_policy_gradient_loss(
    ...
    **kwargs,
    ...
    """Calculate the clipped policy objective and related metrics.
    ...
    metrics["entropy_loss"] = compute_averaged_loss(-log_probs, response_mask, mode=loss_avg_mode)
    ...
    final_pg_loss = compute_averaged_loss(final_pg_loss, response_mask, mode=loss_avg_mode)
...
def calculate_value_function_loss(
    ...
    vf_loss = 0.5 * compute_averaged_loss(clipped_vf_losses, response_mask, mode=loss_avg_mode)
    metrics = {
...
def calculate_divergence(
    ...
    divergence_penalty: Literal["kl", "abs", "mse", "low_var_kl", "full"],
) -> torch.Tensor:
    """Calculate KL divergence given log_probs and ref_log_probs.
    ...
    Args:
        log_probs: torch.Tensor
        ref_log_probs: torch.Tensor
        divergence_penalty: str ("kl", "abs", "mse", "low_var_kl", "full")
    ...
    if divergence_penalty == "kl":
        return log_probs - ref_log_probs

    if divergence_penalty == "abs":
        return (log_probs - ref_log_probs).abs()

    if divergence_penalty == "mse":
    ...
    if divergence_penalty == "full":
        return F.kl_div(ref_log_probs, log_probs, log_target=True, reduction="none").sum(-1)

    raise NotImplementedError(f"Unknown divergence penalty: {divergence_penalty}.")
